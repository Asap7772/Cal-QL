...
class ConservativeSAC(object):
    ...
    def __init__(self, config, policy, qf, qf_sarsa):
        self.config = self.get_default_config(config)
        self.policy = policy
        self.qf = qf
        self.qf_sarsa = qf_sarsa
        ...
        qf_sarsa_params = self.qf_sarsa.init(
            next_rng(self.qf_sarsa.rng_keys()),
            jnp.zeros((10, self.observation_dim)),
            jnp.zeros((10, self.action_dim))
        )
        ...
        self._train_states['qf_sarsa'] = TrainState.create(
            params=qf_sarsa_params,
            tx=optimizer_class(self.config.qf_lr),
            apply_fn=None,
        )
        self._target_qf_params = deepcopy({'qf1': qf1_params, 'qf2': qf2_params, 'qf_sarsa': qf_sarsa_params})
        model_keys = ['policy', 'qf1', 'qf2', 'qf_sarsa']
    ...
    def train(self, batch,  use_cql=True, cql_min_q_weight=5.0, enable_calql=False, sarsa_lb=False):
        self._total_steps += 1
        self._train_states, self._target_qf_params, metrics = self._train_step(
            self._train_states, self._target_qf_params, next_rng(), batch, use_cql, cql_min_q_weight, enable_calql, sarsa_lb
        )
        return metrics

    @partial(jax.jit, static_argnames=('self', 'use_cql', 'cql_min_q_weight', 'enable_calql', 'sarsa_lb'))
    def _train_step(self, train_states, target_qf_params, rng, batch, use_cql=True, cql_min_q_weight=5.0, enable_calql=False, sarsa_lb=False):
        ...
        def loss_fn(train_params):
            ...
            if sarsa_lb:
                ...
                next_actions = jnp.zeros_like(actions)
            dones = batch['dones']
            ...
            @wrap_function_with_rng(rng_generator())
            def forward_qf_sarsa(rng, *args, **kwargs):
                return self.qf_sarsa.apply(
                    *args, **kwargs,
                    rngs=JaxRNG(rng)(self.qf_sarsa.rng_keys())
                )
            ...
            """Sarsa Q function loss """
            q_sarsa_pred = forward_qf_sarsa(train_params['qf_sarsa'], observations, actions)
            q_sarsa_target = jax.lax.stop_gradient(
                rewards + (1. - dones) * self.config.discount * forward_qf_sarsa(target_qf_params['qf_sarsa'], next_observations, next_actions)
            )
            qfsarsa_loss = mse_loss(q_sarsa_pred, q_sarsa_target)
            ...
            loss_collection['qf_sarsa'] = qfsarsa_loss
        ...
        qfsarsa_loss_gradient = jnp.linalg.norm(ravel_pytree(grads[self.model_keys.index("qf_sarsa")]['qf_sarsa'])[0])
        ...
        metrics = collect_jax_metrics(
            aux_values,
            ['log_pi', 'policy_loss', 'qf1_loss', 'qf2_loss', 'qfsarsa_loss', 'alpha_loss',
             'alpha', 'q1_pred', 'q2_pred', 'target_q_values', 'policy_loss_gradient', 'qf1_loss_gradient', 'qf2_loss_gradient', 'qfsarsa_loss_gradient'],
        )
        metrics.update(policy_loss_gradient=policy_loss_gradient, qf1_loss_gradient=qf1_loss_gradient, qf2_loss_gradient=qf2_loss_gradient, qfsarsa_loss_gradient=qfsarsa_loss_gradient)
        metrics.update(use_cql=int(use_cql), enable_calql=int(enable_calql), cql_min_q_weight=cql_min_q_weight, sarsa_lb=int(sarsa_lb))
