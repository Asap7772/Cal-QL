...
FLAGS_DEF = define_flags_with_default(
    ...
    sarsa_lb=False, # Turn on for SARSA-LB
...
def main(argv):
    ...
    if FLAGS.env in ["pen-binary-v0", "door-binary-v0", "relocate-binary-v0"]:
        ...
        dataset = get_d4rl_dataset_with_mc_calculation(FLAGS.env, FLAGS.reward_scale, FLAGS.reward_bias, FLAGS.clip_action, gamma=FLAGS.cql.discount, add_next_actions=FLAGS.sarsa_lb)
        use_goal = False
    ...
    qf_sarsa = FullyConnectedQFunction(observation_dim, action_dim, FLAGS.qf_arch, FLAGS.orthogonal_init)

    if FLAGS.cql.target_entropy >= 0.0:
        FLAGS.cql.target_entropy = -np.prod(eval_sampler.env.action_space.shape).item()

    sac = ConservativeSAC(FLAGS.cql, policy, qf, qf_sarsa)
    ...
    while True:
        ...
        with Timer() as train_timer:
            ...
            for _ in range(n_train_step_per_epoch):
                ...
                train_metrics = prefix_metrics(sac.train(batch, use_cql=use_cql, cql_min_q_weight=cql_min_q_weight, enable_calql=enable_calql, sarsa_lb=sarsa_lb), 'sac')
