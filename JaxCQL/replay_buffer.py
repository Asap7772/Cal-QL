...
def get_d4rl_dataset_with_mc_calculation(env, reward_scale, reward_bias, clip_action, gamma, add_next_actions=False):
    if "antmaze" in env:
        is_sparse_reward=True
    elif 'hopper' in env or 'walker' in env or 'cheetah' in env or 'ant' in env:
        is_sparse_reward=False
    else:
        raise NotImplementedError
    dataset = qlearning_dataset_and_calc_mc(gym.make(env).unwrapped, reward_scale, reward_bias, clip_action, gamma, is_sparse_reward=is_sparse_reward, add_next_actions=add_next_actions)
    ...
    if add_next_actions:
        return_dict['next_actions'] = dataset['next_actions']
    return return_dict
...
def qlearning_dataset_and_calc_mc(env, reward_scale, reward_bias, clip_action, gamma, dataset=None, terminate_on_end=False, is_sparse_reward=True, add_next_actions=False, **kwargs):
    ...
    for i in range(N):
        ...
        if (not terminate_on_end) and final_timestep or i == N-1:
            ...
            if add_next_actions:
                data_['next_actions'].append(dataset['actions'][i+1])
            episode_step += 1
        ...
        if (done_bool or final_timestep) and episode_step > 0:
            ...
            if add_next_actions:
                episode_data['next_actions'] = np.clip(episode_data['next_actions'], -clip_action, clip_action)
